  ...
  useEffect(() => {
    if (user) {
      const hasLiked = favorites.some(
        (favorite) =>
          favorite.property === property._id && favorite.user === user._id
      );
      setLiked(hasLiked);
    }
  ...
  const handleAddFavorite = async () => {
    try {
      if (liked) {
        ...
        if (favorite) {
          ...
          setLiked(
            updatedFavorites.some(
              (fav) => fav.property === property._id && fav.user === user._id
            )
          );
          if (onRemove) {
            onRemove();
        ...
        const newFavorite = await favoriteService.addPropertyToFavorites(
          property._id
        );
        setLiked(true);
